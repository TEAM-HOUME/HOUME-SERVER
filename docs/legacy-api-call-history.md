...
| `request_uri` | 실제 요청 URI path | `/api/v1/mypage/images/123`처럼 path variable 값을 포함하며 query string은 제외 |
| `api_path` | Spring handler의 경로 템플릿 | `/api/v1/mypage/images/{houseId}`처럼 path variable을 포함한 API 경로 |
...
query, 요청 본문, 헤더, IP 주소는 저장하지 않는다. 실제 호출 대상 확인과 API별 호출 여부, 기간 집계, 호출 사용자 확인을 위한 최소 이력이다.
같은 `method + api_path` 호출은 최초 1회만 저장하며, 이후 1시간 동안의 반복 호출은 저장하지 않는다. 따라서 `/my/1`, `/my/2`처럼 path variable 값만 다른 요청도 같은 API 호출로 집계된다. `call_count`는 실제 호출 횟수가 아닌 관찰된 시간 단위 호출 수다.

## Operating Flow

1. 매월 또는 삭제 이슈 검토 시 최근 30일과 90일을 `method`, `api_path` 기준으로 집계한다.
...
group by method, api_path
order by call_count desc, api_path;
```
